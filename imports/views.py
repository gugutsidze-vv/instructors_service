from django.http import response, HttpResponseServerError
    ...
    try:
        ...
        if count != 0:
            ...
            try:
                ...
                sqlstate = ex.args[1]
                output = '<h1>Ой! Ошибочка вышла! ' + \
                    str(sqlstate) + \
                    ' Перезагрузка!!1</h1><script>$(document).ready(function(){await sleep(10000);window.location.reload(true);})</script>'
            cursor = cnxn.cursor()
            ...
            if table_config == 1:
                ...
                output += '<tr>'
                for i in range(0, hours):
                    output += '<th colspan="4" class="hour_th">' + \
                        str(hour)+'</th>'
                    hour = int(hour)+1
                output += '</tr>'
                hour = 9
                ...
                for instructor_name in instructors_name:
                    ...
                    for lesson in instructor_timeline:
                        ...
                        if lesson_start[1] == '00':
                            ...
                            cell_counter_add = 4
                        start_cell = (
                            int(lesson_start[0])-hour)*4+cell_counter_add
            ...
            output = '<h1>Занятий нет</h1>'
            output += '<script>function sleep(ms){return new Promise(resolve=>setTimeout(resolve, ms));}async function carusel(){await sleep(30000);window.location.reload(true);}$(document).ready(function(){carusel()})</script>'
